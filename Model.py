...
def FinalModel(images, watermarks):
    ...
    # Watermarks
    # 4 layer : 1,2,3) CONV => RELU => POOL 4) CL => POOL
    y = Reshape((32, 32, 1), input_shape=(1024,))(watermarks)
    # print(y)

    #  first layer
    y = Conv2D(512, (3, 3), strides=(1, 1))(y)
    y = BatchNormalization(axis=2)(y)
    y = Activation('relu')(y)
    y = AveragePooling2D(pool_size=(2, 2))
    
    model = Model(inputs=watermarks, outputs=y)
    model.summary()


    # Concatenate
#    con = Concatenate([x, y], axis = 2)

FinalModel(np.zeros((3, 512, 512, 3)), np.zeros((3, 32 * 32)))
